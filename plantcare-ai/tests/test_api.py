...
def test_conseil_a_une_source():
    ...
    data = r.json()
    assert data["source"] in ("gemini", "gemini_api", "gabarit_local")
    assert "contexte_utilise" in data
    assert data["contexte_utilise"]["temperature_source"] == "manuel"
    assert data["contexte_utilise"]["humidite_source"] == "manuel"


def test_conseil_accepte_ville_meteo():
    r = client.post("/api/v1/conseil", json={
        "espece": {"id": 1, "nom_sci": "Monstera deliciosa"},
        "mesure": {"sol": 20, "temperature": 25, "lux": 700, "humidite_air": 45},
        "meteo": {"ville": "Paris", "code_pays": "FR", "units": "metric", "lang": "fr"}})
    assert r.status_code == 200
    data = r.json()
    assert "conseil" in data
    assert "source" in data
    assert "meteo_requete" in data
    assert data["meteo_requete"]["mode"] in ("ville", "coordonnees", "aucun")
